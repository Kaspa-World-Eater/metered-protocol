...
Every signed object is encoded identically by both parties or nothing works. This is settled here,
on paper, because a field-order divergence between two encoders has already cost this project once.
...
`worked-example.txt` is generated by `vectors.ts` using this project's own encoder
